...
import { categoryValidations, validate } from '../validations/category.validation.js';
...
router.use((req, res, next) => {
    logger.info(`Category API Request: ${req.method} ${req.path}`, {
        ip: req.ip,
...
// Rate limiters for category operations
...
const deleteCategoryLimiter = createRateLimiter({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 10, // 10 deletes per 15 minutes
    skipSuccessfulRequests: true,
    keyGenerator: (req) => `delete_category_${req.user.id}`,
    redisClient: cacheService.getRedisClient(),
});

const archiveCategoryLimiter = createRateLimiter({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 10, // 10 archive operations per 15 minutes
    skipSuccessfulRequests: true,
    keyGenerator: (req) => `archive_category_${req.user.id}`,
    redisClient: cacheService.getRedisClient(),
});

const restoreCategoryLimiter = createRateLimiter({
    windowMs: 15 * 60 * 1000, // 15 minutes
    max: 10, // 10 restore operations per 15 minutes
    skipSuccessfulRequests: true,
    keyGenerator: (req) => `restore_category_${req.user.id}`,
    redisClient: cacheService.getRedisClient(),
});
...
router.get('/health', (req, res) => {
    metricsCollector.increment('health_check', { service: 'category' });
    res.status(200).json({ status: 'OK', service: 'Category Service', timestamp: new Date().toISOString() });
});
...
router.get(
    '/:id',
    cacheMiddleware(600),
    categoryValidations.getCategoryById,
    validate,
...
router.delete(
    ...
    categoryValidations.deleteCategory,
    validate,
...
router.post(
    ...
    categoryValidations.uploadMedia,
    validate,
    CategoryController.uploadMedia
);
...
router.get(
    '/',
    cacheMiddleware(300),
    categoryValidations.getCategories,
    validate,
...
router.post(
    '/search',
    searchLimiter,
    categoryValidations.searchCategories,
    validate,
...
router.get(
    '/trending',
    cacheMiddleware(300),
    categoryValidations.getTrendingCategories,
    validate,
...
router.post(
    ...
    categoryValidations.bulkCreateCategories,
    validate,
...
router.put(
    '/bulk',
    bulkOperationLimiter,
    categoryValidations.bulkUpdateCategories,
    validate,
...
router.get(
    '/:id/analytics',
    cacheMiddleware(300),
    categoryValidations.getCategoryAnalytics,
    validate,
...
router.get(
    '/:id/export',
    cacheMiddleware(300),
    categoryValidations.exportCategory,
    validate,
...
router.get(
    ...
    categoryValidations.getCategoryStats,
    validate,
...
router.post(
    '/:id/archive',
    archiveCategoryLimiter,
    categoryValidations.archiveCategory,
    validate,
...
router.post(
    '/:id/restore',
    restoreCategoryLimiter,
    categoryValidations.restoreCategory,
    validate,
...
router.get(
    '/:id/audit',
    cacheMiddleware(300),
    categoryValidations.getAuditLogs,
    validate,
    CategoryController.getAuditLogs
);
...
// ERROR HANDLING
// ===========================
const handleCategoryErrors = (err, req, res, next) => {
    logger.error(`Category API Error: ${err.message}`, {
        stack: err.stack,
        path: req.path,
        method: req.method,
        userId: req.user?.id,
...
router.use('*', (req, res) => {
    logger.warn(`Category API 404: ${req.method} ${req.originalUrl}`, {
        ip: req.ip,
        userAgent: req.get('User-Agent'),
        timestamp: new Date().toISOString(),
    });
    ...
    res.status(404).json({
        success: false,
        message: 'Category API endpoint not found',
...
router.use(handleCategoryErrors);
...
if (process.env.NODE_ENV === 'development') {
    router.get('/docs', (req, res) => {
        res.json({
            ...
            version: '1.0.0',
            description: 'RESTful API for managing categories and related operations',
            ...
            routes: {
                ...
                authenticated: [
                    ...
                    'DELETE /:id - Delete category (soft or permanent)',
                    'POST /:id/media - Upload media for category (max 5 files, 5MB each)',
                    'GET / - Get categories with filters and pagination',
                    'POST /search - Search categories with advanced filters',
                    'GET /trending - Get trending categories',
                    'POST /bulk - Bulk create categories (max 100)',
                    'PUT /bulk - Bulk update categories (max 100)',
                    'GET /:id/analytics - Get category analytics',
                    'GET /:id/export - Export category data (JSON or CSV)',
                    'GET /:id/stats - Get category statistics',
                    'POST /:id/archive - Archive category',
                    'POST /:id/restore - Restore archived category',
                    'GET /:id/audit - Get category audit logs',
                ],
                admin: [
                    'GET /admin/:id - Get any category by ID',
                    'GET /admin/:id/audit - Get audit logs for any category',
                ],
            },
            rateLimits: {
                createCategory: '10 requests per 15 minutes per user',
                updateCategory: '20 requests per 5 minutes per user',
                deleteCategory: '10 requests per 15 minutes per user',
                mediaUpload: '10 requests per 10 minutes per user',
                bulkOperations: '5 requests per 30 minutes per user',
                searchCategories: '50 requests per 5 minutes per user',
                archiveCategory: '10 requests per 15 minutes per user',
                restoreCategory: '10 requests per 15 minutes per user',
                admin: '50 requests per 15 minutes per admin',
            },
            validations: {
                createCategory: 'Requires name; optional icon, description, parentId, tags, metadata.isRestricted, metadata.priority',
                getCategoryById: 'Requires id',
                updateCategory: 'Requires id; optional name, icon, description, status.workflow, parentId, tags, metadata.isRestricted, metadata.priority',
                deleteCategory: 'Requires id; optional permanent',
                uploadMedia: 'Requires id, files (max 5, JPEG/PNG/GIF, 5MB each)',
                getCategories: 'Optional page, limit, status, parentId, search, sortBy',
                searchCategories: 'Requires query; optional filters (status, parentId, tags), page, limit',
                getTrendingCategories: 'Optional timeframe (7d, 30d, 90d), parentId, limit (max 50)',
                bulkCreateCategories: 'Requires categories array (1-100 items); each with name; optional icon, description, parentId, tags, metadata.isRestricted, metadata.priority',
                bulkUpdateCategories: 'Requires updates array (1-100 items); each with id, data (optional name, icon, description, status.workflow, parentId, tags, metadata.isRestricted, metadata.priority)',
                getCategoryAnalytics: 'Requires id; optional timeframe (7d, 30d, 90d)',
                exportCategory: 'Requires id; optional format (json, csv)',
                getCategoryStats: 'Requires id',
                archiveCategory: 'Requires id',
                restoreCategory: 'Requires id',
...
const checkCacheConsistency = async (req, res, next) => {
    if (['POST', 'PUT', 'DELETE'].includes(req.method)) {
        const cacheKeys = await cacheService.keys(`category:${req.params.id}:*`);
        if (cacheKeys.length > 0) {
            logger.info(`Clearing ${cacheKeys.length} cache entries for category ${req.params.id}`, {
                path: req.path,
                userId: req.user?.id,
            });
            await cacheService.deletePattern(`category:${req.params.id}:*`);
...
router.use(['/:id', '/:id/media', '/:id/archive', '/:id/restore'], checkCacheConsistency);
...
router.use('/bulk', (req, res, next) => {
    logger.info(`Bulk operation initiated`, {
        ...req.logContext,
        itemCount: req.body.categories?.length || req.body.updates?.length || 0,
    });
